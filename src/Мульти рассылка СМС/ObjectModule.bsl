...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Версия"			, "1.0.0.54");
...
Функция _PROSTOSMS_СписокОшибок() Экспорт
	
	Список = Новый СписокЗначений;
	
	Список.Добавить(0, "Сообщение принято для отправки");
	Список.Добавить(3, "API на Вашем аккаунте отключено. Обратитесь в поддержку");
	Список.Добавить(7, "Заданы не все необходимые параметры");
	Список.Добавить(99, "Транзакция отправки SMS не прошла");
	Список.Добавить(602, "Пользователя не существует. Проверьте правильность логина/пароля или api key. Ошибка также может возникать в случае если изменился ip адрес с которого направляются запросы");
	Список.Добавить(602, "SMS не существует");
	Список.Добавить(605, "Пользователь заблокирован");
	Список.Добавить(607, "Имя отправителя недопустимо");
	Список.Добавить(608, "Недопустимая длина имени отправителя");
	Список.Добавить(609, "Сотовый оператор не подключен");
	Список.Добавить(610, "Не подключен тариф пользователю");
	Список.Добавить(611, "Не корректно установлена стоимость SMS");
	Список.Добавить(617, "Неверный формат номера получателя SMS");
	Список.Добавить(618, "Номер aбонента в Черном списке");
	Список.Добавить(622, "Номер aбонента в Черном списке");
	Список.Добавить(620, "Имя отправителя должно быть на латинице");
	Список.Добавить(621, "В имени отправителе более 2-цифр или общая длина менее 4-х символов, что не допустимо");
	Список.Добавить(623, "Не достаточно средств. Пополните баланс");
	Список.Добавить(624, "Обнаружены запрещенные слова в тексте сообщения. Обратитесь в поддержку");
	Список.Добавить(625, "Не удалось определить направление отправки SMS");
	Список.Добавить(626, "Не удалось получить данные по API KEY");
	Список.Добавить(627, "Неверный API KEY");
	Список.Добавить(628, "В рамках тестирования сервиса отправка разрешена только на номер, который привязан к личному кабинету (раздел ""Настройки"" - ""Контактная информация и общие настройки""). Заполните реквизиты компании в разделе ""Настройка"" - ""Реквизиты компании"", чтобы активировать полный функционал сервиса");
	Список.Добавить(629, "Превышен лимит отправки номеров в одном запросе (допускается не более 1000 номеров в запросе)");
	Список.Добавить(631, "Сообщение отклонено. Нельзя смешивать русские и английские символы в одном слове. Сервис не принимает SMS, в которых в одном слове смешаны кириллица и латиница, т.к. операторы блокируют такие смс, воспринимая за попытку обойти СПАМ фильтр");
	Список.Добавить(632, "Незарегистрированный sender_name. Необходимо добавить отправитель в раздел ""Отправители"" в личном кабинете и в обязательном порядке подать на согласование с оператором ПАО ""Мегафон""");
	Список.Добавить(645, "Сообщение отклонено.");
	Список.Добавить(699, "Не удалось установить соединение.");
	
	Возврат Список;
	
КонецФункции

Функция _PROSTOSMS_СписокСтатусовДоставкиСообщений() Экспорт
	
	Список = Новый СписокЗначений;
	
	Список.Добавить(0	, "Отправлено");
	Список.Добавить(1	, "Доставлено");
	Список.Добавить(2	, "Не доставлено");
	Список.Добавить(16	, "Не доставлено в SMSC");
	Список.Добавить(34	, "Не доставлено (просрочено)");
...
Функция _PROSTOSMS_ОтправитьSMS(ТЗ, ПосчитатьСтоимость = Ложь) Экспорт
	...
	СписокОшибок = _PROSTOSMS_СписокОшибок();
	КоличествоИЛИСтоимостьОтправленных = 0;
	...
	Для Каждого Стр Из ТЗ Цикл		
		Если Стр.Флаг Тогда
			...
			КонецЕсли;
	        СМС		= УдалитьНеПечатныеСимволы(СМС); 
			Ответ	= _PROSTOSMS_ПослатьКоманду("method=push_msg&ver=1C_softonit",
				"text=" + (СМС) 
				...
				Если Ответ <> Неопределено И Ответ <> "" Тогда  
						...
						Иначе 
							...
							Ошибка = СписокОшибок.НайтиПоЗначению(Число(КодОшибки));
							Стр.Статус	= ?(Ошибка <> Неопределено, Ошибка.Представление, "Ошибка отправки SMS (" + ТекстОшибки + ")");							
						КонецЕсли;
				КонецЕсли;                                           
...
Функция _PROSTOSMS_ОбработатьРезультатОтвета(Ответ, Тэг) Экспорт
	...
	ТэгНачала				= "<" + Тэг + ">";
	ТэгОкончания			= "</" + Тэг + ">";
	ЛевИндекс				= СтрНайти(Ответ, ТэгНачала);
	ЛевИндекс				= ЛевИндекс + СтрДлина(ТэгНачала);
	ПравИндекс				= СтрНайти(Ответ, ТэгОкончания);
...
Функция _PROSTOSMS_ПослатьКоманду(Команда, Аргументы = "") Экспорт    
	...
	Иначе
		ДанныеАвторизации = "&email=" + СокрЛП(URLEncode(Логин)) + "&password=" + СокрЛП(URLEncode(Пароль));
	КонецЕсли;
...
Процедура _PROSTOSMS_Баланс(Баланс, ЕстьОшибка) Экспорт
	...
	Если Ответ <> Неопределено И Ответ <> "" Тогда  
		...
		Иначе 
			...
			ИтогОшибки	= ТекстОшибки + "(" + КодОшибки + ")";			
			Ошибка = СписокОшибок.НайтиПоЗначению(Число(КодОшибки));
			Баланс	= ?(Ошибка <> Неопределено, Ошибка.Представление, "Не известный номер ошибки (" + ИтогОшибки + ")");							
...
Процедура _PROSTOSMS_ОбновитьСтатусДоставки(ТЗ) Экспорт
	...
	Статусы = _PROSTOSMS_СписокСтатусовДоставкиСообщений();
	...
	Для Каждого Стр Из ТЗ Цикл
		...
		Если Ответ <> Неопределено И Ответ <> "" Тогда
			КодОшибки = _PROSTOSMS_ОбработатьРезультатОтвета(Ответ, "err_code");
			Если КодОшибки = "0" Тогда
				СтатусСтрока = _PROSTOSMS_ОбработатьРезультатОтвета(Ответ, "state");
				Статус	= Число(СтатусСтрока);
				Элемент	= Статусы.НайтиПоЗначению(Статус);
				Если Элемент <> Неопределено Тогда
					Стр.Статус	= Элемент.Представление;
				Иначе
