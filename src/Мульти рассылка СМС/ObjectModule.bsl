...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Версия"			, "1.0.0.37");
	...
	Если мПараметры <> Неопределено Тогда
		...
		ПараметрыРегистрации.Вставить("СсылкаНаСайт",				мПараметрыСервиса.СсылкаНаСайт);
		ПараметрыРегистрации.Вставить("СсылкаНаСайтПредставление",	мПараметрыСервиса.СсылкаНаСайтПредставление);
		ПараметрыРегистрации.Вставить("Пользователь",				мПараметры.SMS_Логин);
		ПараметрыРегистрации.Вставить("Логин",						мПараметры.SMS_Логин);
		ПараметрыРегистрации.Вставить("Пароль",						мПараметры.SMS_Пароль);
		ПараметрыРегистрации.Вставить("Отправитель",				мПараметры.SMS_ИмяОтправителя);
		ПараметрыРегистрации.Вставить("КлючAPI",					мПараметры.SMS_КлючAPI);
	Иначе 
...
Функция _SMSRU_СписокОшибок() Экспорт
	...
	Список.Добавить("213", "Указано более 100 номеров в списке получателей");
...
Функция _SMSRU_СписокОтправителей() Экспорт
	...
	Иначе
		...
		Иначе
			СтрокаОшибки	= ?(Ошибка <> Неопределено, Ошибка.Представление, "Не известный номер ошибки проверки баланса (" + Строка(ПерваяСтрока) + ")");
		КонецЕсли;
...
Функция _TERASMSRU_ПослатьКоманду(Команда, Аргументы = "") Экспорт
	...
	Ресурс = Команда;                         
    _Параметры = "login=" + СокрЛП(URLEncode(Логин)) + "&password=" + СокрЛП(URLEncode(Пароль)) + "&source=multisms" +
		?(Не ПустаяСтрока(Аргументы), "&" + СокрЛП(Аргументы), "");
...
Функция _TERASMSRU_ОтправитьSMS(ТЗ, ПосчитатьСтоимость = Ложь) Экспорт
	...
	Для каждого Стр Из ТЗ Цикл		
		Если Стр.Флаг Тогда
			...
			Ответ	= _TERASMSRU_ПослатьКоманду("/outbox/send/", "target=" + URLEncode(Стр.Телефон) + "&message=" + URLEncode(СМС) + ?(ПустаяСтрока(Отправитель), "", "&sender=" + URLEncode(Отправитель)) + ?(ОтправлятьВУказанноеВремя, "&timezone=1&date_schedule=" +  URLEncode(Формат(ДатаИВремяОтправки, "ДФ='yyyy-MM-dd HH:mm:ss'")), "")); 
...
Функция ОтправитьSMS(ТекстSMS, ПолучателиSMS, ВыдаватьСообщенияОбОшибке = Истина) Экспорт
	...
	ИначеЕсли Сервис = "terasms.ru" Тогда 
		Результат = ОтправитьSMSСервисterasms(ТекстSMS, ПолучателиSMS, ВыдаватьСообщенияОбОшибке = Истина, мКлючAPI, мКанал);
	КонецЕсли;
...
Функция ПолучитьСтатусSMS(ПолучателиSMS, ВыдаватьСообщенияОбОшибке = Истина) Экспорт
	...
	ИначеЕсли Сервис = "terasms.ru" Тогда 
		Результат = ПолучитьСтатусterasmsru(ПолучателиSMS, ВыдаватьСообщенияОбОшибке = Истина);
	КонецЕсли;
...
Функция ОтправитьSMSСервисterasms(ТекстSMS, ПолучателиSMS, ВыдаватьСообщенияОбОшибке = Истина, мКлючAPI, мКанал);
	...
	Для Каждого ПолучательSMS Из ПолучателиSMS Цикл		
		...
		Ответ	= _TERASMSRU_ПослатьКоманду("/outbox/send/", "target=" + URLEncode(ПолучательSMS.Телефон) + "&message=" + URLEncode(СМС) + ?(ПустаяСтрока(Отправитель), "", "&sender=" + URLEncode(Отправитель))); 
...
Функция ПолучитьСтатусterasmsru(ПолучателиSMS, ВыдаватьСообщенияОбОшибке = Истина) Экспорт
	...
	Для Каждого ПолучательSMS Из ПолучателиSMS Цикл
		...
		Ответ = _TERASMSRU_ПослатьКоманду("/outbox/status/", "&id=" + URLEncode(ПолучательSMS.IDСообщения));
		// 255753949:440036316:12,255753949:440036317:12
		МассивРазбиений = РазложитьСтрокуВМассивПодстрок(Ответ, ",");
		Для Каждого Эл Из МассивРазбиений Цикл		
			Массив = РазложитьСтрокуВМассивПодстрок(Эл, ":");
